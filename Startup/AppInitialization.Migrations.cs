    ...
    { 
        ...
        private static void EnsureDatabase(string databaseKey)
        {
            ...
            using (var serverConnection = SqlConnections.New(cb.ConnectionString, cs.ProviderName))
            {
                ...
                if (isLocalServer)
                {
                    var filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, catalog);
        ...
        private static void RunMigrations(string databaseKey)
        {
            ...
            using (var sw = new StringWriter())
            {
                ...
                var runner = new RunnerContext(announcer)
                {
                    ...
                    Targets = new string[] { typeof(AppInitialization).Assembly.Location },
                    Task = "migrate:up",
                    WorkingDirectory = Path.GetDirectoryName(typeof(AppInitialization).Assembly.Location),
